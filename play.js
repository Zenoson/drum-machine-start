...
function playSound(sound) {
    var audElm = document.querySelector("audio[data-sound='".concat(sound.toLowerCase(), "']"));
...
document.addEventListener("keydown", function (event) {
    ...
    if (event.key == "/")
        ...
        document.querySelectorAll(".history").forEach(function (elem) {
            elem.remove();
        });
    }
    var drumElm = document.querySelector(".drum[data-key='".concat(event.key, "']")) || null;
    if (!drumElm) {
        return;
    }
...
document.addEventListener("keyup", function (event) {
    if (event.repeat)
        return;
    var playingElm = document.querySelector(".drum[data-key='".concat(event.key, "']")) || null;
    if (!playingElm)
        return;
...
function handleClickDrum(event) {
    var drumClicked = event.currentTarget;
